...
function stars(rating) {
    ...
    if (half) result += '⯨';
    for (let i = 0; i < (5 - full - (half ? 1 : 0)); i++) result += '☆';
    return result;
}
            ...
            cards += `
                <div style="background:#f9fafb;padding:1.5rem;border-radius:12px;margin-bottom:1rem;border-right:4px solid #667eea">
                    <div style="display:flex;justify-content:space-between;margin-bottom:0.5rem">
                        <strong style="color:#1f2937">${r.reviewer_name || 'عميل'}</strong>
                        <span style="color:#fbbf24;font-size:1.2rem">${stars(r.rating || 5)}</span>
                    </div>
                    ${r.verified_purchase ? '<div style="color:#059669;font-size:0.8rem;margin-bottom:0.5rem">✓ عميل موثق</div>' : ''}
                    <p style="margin:0;color:#374151;line-height:1.6">${r.comment || 'منتج ممتاز'}</p>
                    <div style="color:#9ca3af;font-size:0.85rem;margin-top:0.5rem">${r.date || '2025-10-01'}</div>
                </div>
            `;
        });
            ...
            <div style="background:white;padding:2.5rem;border-radius:16px;margin-top:3rem;box-shadow:0 10px 30px rgba(0,0,0,0.06)">
                <div style="margin-bottom:2rem;padding-bottom:1.5rem;border-bottom:2px solid #f3f4f6">
                    ...
                    <div style="display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap">
                        <div style="font-size:3rem;font-weight:900;color:#f59e0b">${p.rating || 4.8}</div>
                        <div>
                            <div style="font-size:1.5rem;color:#fbbf24">${stars(p.rating || 4.8)}</div>
                            <div style="color:#6b7280">${p.review_count || 0} تقييم</div>
                        </div>
                    </div>
                </div>
                ${cards}
...
<body>
    ...
    <div class="container">
        <div class="product-layout">
            <div><img class="main-image" src="${p.image_link}" alt="${p.title}"></div>
            <div>
                <h1>${p.title}</h1>
                <div style="margin-bottom:1rem;color:#fbbf24;font-size:1.5rem">${stars(p.rating || 4.8)} <span style="color:#666">(${p.review_count || 0} تقييم)</span></div>
                <div class="price-box">
...
console.log(`\n✅ تم! ${count} صفحة بتقييمات HTML منسقة\n`);
