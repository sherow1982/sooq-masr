...
body {font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background:linear-gradient(135deg,#f5f7fa 0%,#c3cfe2 100%);min-height:100vh;padding:20px;direction:rtl;}
...
.breadcrumb {padding:20px 30px;background:#f8f9fa;border-bottom:1px solid #e0e0e0;}
...
@media (max-width:968px) {.product-main {grid-template-columns:1fr;}}
.product-image-section {position:sticky;top:20px;height:fit-content;}
.main-image {width:100%;border-radius:15px;box-shadow:0 10px 30px rgba(0,0,0,0.15);transition:transform 0.3s;}
.main-image:hover {transform:scale(1.02);}
.discount-badge {position:absolute;top:20px;right:20px;background:linear-gradient(135deg,#f093fb 0%,#f5576c 100%);color:white;padding:10px 20px;border-radius:25px;font-weight:bold;font-size:1.2em;box-shadow:0 5px 15px rgba(245,87,108,0.4);}
.product-info {padding:20px;}
.product-title {font-size:2em;color:#2d3436;margin-bottom:15px;line-height:1.4;}
.rating-section {display:flex;align-items:center;gap:15px;margin-bottom:20px;padding:15px;background:linear-gradient(135deg,#ffecd2 0%,#fcb69f 100%);border-radius:10px;}
.stars {color:#ffd700;font-size:1.5em;text-shadow:1px 1px 2px rgba(0,0,0,0.1);}
.rating-number {font-size:1.8em;font-weight:bold;color:#2d3436;}
.review-count {color:#636e72;}
.price-section {background:linear-gradient(135deg,#a8edea 0%,#fed6e3 100%);padding:25px;border-radius:15px;margin:25px 0;}
.price-row {display:flex;align-items:center;gap:20px;flex-wrap:wrap;}
.current-price {font-size:3em;color:#00b894;font-weight:bold;}
.old-price {font-size:1.8em;color:#b2bec3;text-decoration:line-through;}
.save-amount {background:#00b894;color:white;padding:8px 15px;border-radius:20px;font-weight:bold;}
.description {background:#f8f9fa;padding:25px;border-radius:15px;margin:25px 0;line-height:1.8;font-size:1.1em;color:#2d3436;}
...
.rating-summary {background:white;padding:30px;border-radius:15px;margin-bottom:30px;box-shadow:0 5px 15px rgba(0,0,0,0.1);}
.rating-bar-row {display:flex;align-items:center;gap:15px;margin:10px 0;}
.rating-bar-container {flex:1;height:10px;background:#e0e0e0;border-radius:5px;overflow:hidden;}
.rating-bar-fill {height:100%;background:linear-gradient(90deg,#ffd700,#ffa500);transition:width 0.5s;}
.review-card {background:white;padding:25px;border-radius:15px;margin-bottom:20px;box-shadow:0 5px 15px rgba(0,0,0,0.08);transition:transform 0.3s;}
...
.reviewer-avatar {width:50px;height:50px;border-radius:50%;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;display:flex;align-items:center;justify-content:center;font-size:1.5em;font-weight:bold;}
.reviewer-name {font-weight:bold;color:#2d3436;font-size:1.1em;}
.review-date {color:#636e72;font-size:0.9em;}
...
def generate_product_page(product):
    ...
    for review in product['reviews']:
        ...
        reviews_html += f'''<div class="review-card"><div class="review-header"><div class="reviewer-info"><div class="reviewer-avatar">{review['reviewer_name'][0]}</div><div><div class="reviewer-name">{review['reviewer_name']}</div><div class="review-date">{review['date']}</div></div></div><div class="review-rating"><span class="stars">{stars}</span><span class="verified">{verified}</span></div></div><div class="review-comment">{review['comment']}</div></div>'''
    ...
    for i in range(5, 0, -1):
        ...
        rating_bars += f'<div class="rating-bar-row"><span>{i} نجمة</span><div class="rating-bar-container"><div class="rating-bar-fill" style="width:{percentage}%"></div></div><span>{count}</span></div>'
    ...
    product_title_clean = product['title'].replace('"', '').replace('\t', '').replace("'", "")
...
<body>
<div class="container">
<div class="header"><h1>🛍️ سوق مصر</h1><p>منتجات أصلية بأفضل الأسعار</p></div>
<div class="breadcrumb"><a href="../index.html">الرئيسية</a> / <a href="../products.html">المنتجات</a> / <span>{product['title']}</span></div>
<div class="product-main">
<div class="product-image-section"><div style="position:relative;"><img src="{product['image_link']}" alt="{product['title']}" class="main-image"><div class="discount-badge">-{discount}%</div></div></div>
<div class="product-info">
<h1 class="product-title">{product['title']}</h1>
<div class="rating-section"><span class="stars">{stars}</span><span class="rating-number">{product['rating']}</span><span class="review-count">({product['review_count']} تقييم)</span></div>
...
def main():
    ...
    for product in products:
        try:
            ...
            print(f'❌ خطأ: {product["id"]}: {str(e)}')
    
    print(f'\n✨ اكتمل! تم إنشاء {success_count} صفحة')
